...
```json
{
  ...
  "zones": [
    ...
    { "key": "seat_box_north", "layout": "stack", "pos": [0.02, 0.02, 0.2, 0.28] },
...
| `grave` | Where a card **standing here** dies to, when it is not the grave a game declares once with `status`. Third of the five places `destroy:` asks, so a card's own `grave` and its tags still win — this is for a place whose deaths are its own business, and a zone naming itself is how "whatever dies here is already where it goes" is said |
...
| `arrives` | What this zone does when a card comes **into play** here, as opposed to merely landing — the counterpart to a card's `leaves`, keeping the same rule at the other end. `leaves` fires on the way *out of play* and a unit walking between two board zones fires nothing; this fires on the way *in*, and a unit walking back fires nothing either. Codex's combat is a move out to the duel zone and home again, so an arrival trigger that answered that would fire on every attack. A card lent to a question comes home having arrived nowhere, because the zone it counts as coming from is the one it was **borrowed from** — the same substitution `leaves` makes. A created token arrives too, where `receive` is not fired by a create and never was. Takes `{ needs, action }`, asked with the **arriving card** as `@self`: `receive` is the place doing something about what landed in it, and this is a card's arrival being announced, which is what lets `emit` name the newcomer and `others` leave it out of a pool. Only meaningful on a `status: "board"` zone, and the validator says so |
...
{ "key": "char_jaina", "text": "Jaina", "tags": ["immutable"],
  "play": { "action": ["create:mine.bag:playing_with_fire:1",
                       ...
                       "set_owner:self:mine.player", "move_to:mine.fighter"] } }
```
...
**order** it is in, and nothing else:

```
lowest:tier.enemy.patrol        their patrollers, cheapest tech first
highest:atk.mine.army           my army, biggest attacker first
lowest:price.enemy.patrol.unit  the cheapest unit at their posts
```

They carry the number they sort by — the spelling a phase's `order` already uses
— and they sit in the quantifier's slot, so the rest of a scope reads exactly as
it did: an owner word may follow, then a zone or a tag, and `<zone>.<tag>` still
narrows.
...
a pool that has been put in order. Nothing about counting changed.

**Ties keep the order the pool was already in**, so a seeded replay is a replay.
And a card carrying no such number reads nought, which puts it at the front of
`lowest` — the one reading that needs no rule to remember.
...
`each_seat:` wraps any action, including one that asks:

```json
"action": ["each_seat:show:mine.hand:optional"]
```

There is one `options` zone and one overlay over it, so the questions do not all
open at once — **the second and third are written down and asked when the one
before them is answered.** Each is asked of the seat that asked it, so `mine`
...
emptied opens nothing at all, which is the same rule as an empty offer anywhere
else.

That is why a question asked of each seat needs no phase of its own. A question
...
| `set_owner:<scope>:<whose>` | Hand those cards to whoever the second scope belongs to, or to nobody (`none`). Whose is named the way `set_active_seat` and `set_priority` name it — an ordinary scope, resolved to a card, and the seat is whose that card is — so `mine.player`, `opponent` and `enemy.player` say here what they say everywhere else. `none` is its own word because it is not a seat: an empty scope means *skip*, so nobody could be named by one. Whose a card is is settled when it is dealt and stays settled, so this is the only thing that changes it: mind control, and a pile that disowns whatever lands in it |
...
| `reset:<scope>[:<stat>]` | **Put the numbers back to what the card is printed with** — every stat the template declares, or one of them named. A card that comes back into play comes back at its printed values, and saying that by hand was a `stat_set` per number, spelling out what `card_stats` already says three lines above: Codex's eighteen hero summons were six each. A stat's own `start` folds into `card_stats` at load, so "every fighter begins at nought" restores through here without the card repeating it. The ceiling and the floor come back with the value, so a card whose maximum was raised comes back down to its printed one. It does not touch what was never the card's: a buff belongs to a tag and leaves when the tag does, and being spent has its own word in `ready` |
