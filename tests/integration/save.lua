...
function M.test_save_the_menu_offers_continue_only_when_there_is_one(check)
	save.forget("quick")
	flow.init("menu.json", 7)
	local cont = find("m_continue", "start")
	...
	flow.init("chess.json", 7)
	local btn = find("sys_save", "menu")
	check("the system column deals a Save button", btn ~= nil)
	flow.use_system_card(btn.id)
	check("playing it saves", save.exists("quick"))
	local want = net.fingerprint()

	flow.init("menu.json", 7)
	cont = find("m_continue", "start")
...
function M.test_save_a_loaded_game_keeps_its_log_and_loses_its_undo(check)
	...
	flow.use_system_card(find("sys_save", "menu").id)
	check("there is something to undo before the load", flow.can_undo())
