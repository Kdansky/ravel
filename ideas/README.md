...
| [03](03-a-move-out-of-a-stock.md) | A move out of a stock | **shipped** — `take` is `move` for a source that counts instead of keeps, and the count on `move` went in ahead of it. 95 pairs in Puzzle Strike became 95 statements. Left: where a destroyed card goes, which is [28](28-a-zone-by-its-parts.md)'s |
