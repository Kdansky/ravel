...
| 1 | [07](07-presentation.md) — **two marks the draw path gets wrong** | medium | small | two left-overs the last pass named and did not take, and both are visible in a shipped game. A grid still covers its own label — its cells come from `zones.cell_rect` and not from `card_places`, so LoR's *Bench* disappears under the first unit played into it, which is a named zone losing its name exactly when there is most to work out. And a badge always draws an icon, so Splendor's six token piles wear the fallback diamond beside their count: clutter rather than a lie, and underneath it **a badge that is only a number has no spelling**. One pass against the screenshot harness, in two modules; 07 now says where each lands and what the grid one has to route around (`zones.lua` has no font)
