...
| [17](17-conditions-as-expressions.md) | A condition is one string | large | **the reading half is shipped** (`e2ded7d`) — `"gold >= 3"` is a condition anywhere one is taken, additively, and the two spellings are held equal by a 120-case matrix. Arithmetic stays refused, and the file says which half of the job it belongs to. Left: the migration, then deleting the old shapes |
...
| [22](22-the-crew.md) | The Crew: The Quest for Planet Nine | medium | **researched, and half its one primitive is shipped**: `@owner_of.<scope>` reads whose a card is (`15e3c3f`). Left is the writing half — making that seat the active one — and then the game itself, which is content |
...
| 1 | [22](22-the-crew.md) — **the seat a scope points at, writing half** | medium | small | **half shipped**: `@owner_of.<scope>` reads it (`15e3c3f`) — the seats owning what a scope names, answered as their own cards, with the acting card's own seat as the bare form. What is left is `set_active_seat:<scope>`, a `G.seat_index` lookup over the answer the scope already gives, and it is what The Crew cannot be built without: the trick winner leads the next trick, and LoR's attack token holder is supposed to act first. Cheapest thing on the board that unblocks a whole game |
| 2 | [17](17-conditions-as-expressions.md) steps 3–4 — **the migration, then delete the old shapes** | medium | medium | the reading half landed **additively**, so nothing is fixed yet: findings 4, 5 and 6 are only fixed when the struct spellings go, and until then the format has *four* shapes where it had three. The golden traces are the proof and the work is one game per commit. It also carries the fold-in `AUTHORING.md` never got — the string form is documented today only in `ideas/17`, which means the spelling shipped where a game author cannot find it |
| 3 | [06](06-schema-and-types.md) gap 5 — **a tag is a boolean below the door** | low | small | no longer rides along with 2 — the door pass is spent. Stands alone and is mostly shipped already: a zone entity's `tags` *is* the boolean map, and only two card readers still walk the array. One of them (`cards.home_zone`) depends on tag **order**, so the real deliverable is a validation error for two tags claiming a home zone, not a representation change |
| 4 | [23](23-splendor.md) — **Splendor, from the data already collected** | medium | small | the cheapest whole game on the board: zero new primitives, zero cut content, and all 90 cards plus 10 nobles already triangulated across three sources. It is the strongest available answer to "does the vocabulary reach a euro nobody designed it for", for the price of a JSON file and a scripted test. After 2 so its conditions are written once, in the spelling that survives — though the argument is weaker than it was, most of Splendor's numbers being costs, and a cost is staying a map |
