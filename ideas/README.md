...
| 8 | [35](35-a-board-that-fits-the-game.md) — **the seat's own name** | small + one decision | *"Player One to answer"* is what the react bar prints all game, because `seat_name` returns the seat card's `text` and Spellstorm's seat cards say exactly that. `{owner}` shipped and cannot reach it — something has to rename the *seat*. Three shapes and none picked: `seat_name` falling through to a zone the seat owns, a `name:` verb, or `transform` with per-wizard seat cards. Highest payoff per line of anything open, and blocked only on the decision |
