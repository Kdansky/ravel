...
| [42](42-an-if-inside-an-action-list.md) | An if inside an action list | **open, blocked on the grammar.** A condition may contain a colon, so `when:<cond>:<action>` has no reading. 44 Spellstorm abilities and 9 Codex ones exist only to carry a gate |
