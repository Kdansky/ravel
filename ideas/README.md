...
| [41](41-a-name-for-a-list-of-actions.md) | A name for a list of actions | **shipped as a verb with a body** — `action` in place of `does`, arguments as `param1`… by position, stat changes inside it the verb's own so `adjusts` reaches them. Left: the gated routines a body cannot say, which can now use [42](42-an-if-inside-an-action-list.md)'s if |
| [42](42-an-if-inside-an-action-list.md) | An if inside an action list | **shipped** as `{ "if": [...], "do": [...] }` in the list, and Spellstorm's `cast2`/`cast3` moved into it. Left: `activate_zone` still reads a phase-run ability's `needs` as an if — ~150 abilities across six games that now have somewhere to go |
