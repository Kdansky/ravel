...
| 6 | [31](31-either-of-two.md) — **`or` between conditions** | small, and now with customers | **moved up: the honest customers turned up.** Removing Puzzle Strike's buy ceiling left two faults that are one condition each and cannot be written as one comparison — the borrowed shopping Rigorous Training and Upgrade still hand over as a purse, which a reaction cannot replace with an offer — the announcement it was answering is gone by the time the pick resolves — and the rulebook's own escape from the buy-one rule when the Wound stack is empty, which currently deadlocks the turn. `meets_all` is the one function fourteen consumers share, so whatever spelling wins lands once |
