...
| [01](01-boardgames.md) | Any board game as JSON | **the ladder.** Lost Cities, chess and checkers shipped. The jumped square needed no word — a pattern is already a scope, and the anchor follows the piece — and the chain turned out to be a route back to the same seat. Left: the button that ends a chain, Klondike's run moves, and triggers |
...
| [44](44-ghost-stories.md) | Ghost Stories | **built and playing**, four seats at Initiation level, all 65 cards. Village and boards are one 5x5 grid, so reach and movement are two patterns and no arithmetic; a Taoist rides its tile. Left: the two-ghost exorcism, tokens lent across a tile, the eight Taoist powers, and neutral boards for fewer than four players |
...
| 79 | [01](01-boardgames.md) gap 1 — **an amount that may ask `aims:`** | small, and no word to agree | checkers ships with a *Done jumping* button because a chain cannot end itself, and everything needed to delete it is built: `aims:jump_right` answers 1 after the first jump of a double and 0 after the second, measured from the square just landed on. An **amount** takes a number, a `count:`/`sum:` measure or a compute and refuses `aims:` by name; a compute is bound before the action runs, so it would measure from the square the piece has not left yet. Widening the amount grammar is the whole fix, and it makes a chain *forced* — half the forced-capture rule for nothing. One customer, and it deletes a button rather than adding a word |
