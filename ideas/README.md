...
| [41](41-a-name-for-a-list-of-actions.md) | A name for a list of actions | **shipped as a verb with a body** — `action` in place of `does`, arguments as `param1`… by position, stat changes inside it the verb's own so `adjusts` reaches them. Left: the gated routines a body cannot say, until a verb takes gates ([46](46-one-list-of-conditions.md)) |
...
| [46](46-one-list-of-conditions.md) | One list of conditions | **shipped** — `needs` keyed by what failing it does: `req`, `where` (moved off targets, move rules and reveals), `fizzle`, `event`, and named gates that `name?` lines sit behind, asked once and kept. Replaced the inline if/do, and fixed Swamp Silt. Left: a verb that takes gates |
...
|---|---|---|---|
| 77 | [41](41-a-name-for-a-list-of-actions.md) — **the gated routines left in rules zones** | medium, a migration, and one word | once per point and a verb over a kind needed no word. What is left is every routine behind a condition, which can move into a verb once a verb takes a `needs` of gates ([46](46-one-list-of-conditions.md)) |
