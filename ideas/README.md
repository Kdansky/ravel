...
| [34](34-an-opponent.md) | An opponent | **not started.** The board-game kind: a seat the engine plays. `legal_moves()` already exists in the test harness and plays whole games at random, so the first version is moving it out; everything after it waits on a game file being able to say what winning looks like as a *number* |
...
| 20 | [34](34-an-opponent.md) — **`legal_moves` out of the test harness** | small | the first step of an opponent, and free: the function already plays every game to the end at random, and moving it costs no format word. What it unblocks is a game tried by somebody other than one person moving both seats. The rest of that track waits on a way for a game file to say what winning is as a number |
| 33 | [arnak](arnak/design.md) — **who spent a card's exhaust** | small, and a word to agree first | the engine records that a card is exhausted and not by whom, so Arnak's Fear rule — *your* archaeologist coming home from a guarded site — cannot be written, and the file diverges instead. A stat written on exhaustion the way `last_acted` is written on play closes it. Low because it is one rule in one game and it costs a word |
| 15 | [01](01-boardgames.md) gap 1 — **the square a move passes over** | medium | a jump takes the piece it flies past, and nothing can name that square. Castling-through-check asks for the same word; en passant no longer does, having shipped as `where` |
