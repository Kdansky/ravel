...
| [37](37-codex.md) | Codex | **built and playing**, all 330 cards in the file: red, green and blue run, black, white, purple and the two neutral specs are scaffolding. Sacrifice is closed both ways, and so is the Truth spec: an Illusion dies of being aimed at, which is one line on a tag now. Three heroes a side deal now. Left: the tech II spec lock that makes it the rulebook's game; Macciatus and the two cards that hand the word out — an owner scope anchored on `@self`, and granting a tag at all — forecast, and three small words the box asks for 33 times between them |
...
|---|---|---|---|
| 77 | [41](41-a-name-for-a-list-of-actions.md) — **what verb bodies cannot say yet** | medium, and a design first | four games moved onto verbs with no change in play. What is left in rules zones is what a body cannot say: a routine behind a `needs`, which is [42](42-an-if-inside-an-action-list.md)'s if and sorts after 78. `GIVE(kind)` waits on a parameter being a whole word. Once per point of something needed no word — `copy`'s count reads a stat, and Croh's four gated cards are one |
...
| 85 | [37](37-codex.md) — **the tech II spec lock** | medium, no word to agree | three heroes a side shipped. What makes it the 3-hero game rather than one with three heroes is that tech II declares a spec and only that spec's tech II and III may be built — `t2_ok` does not read it yet, so all three are buildable. Tech lab and heroes' hall ride on it, and a hero's death still hands out two splittable free levels where the rule names one hero |
