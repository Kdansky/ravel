...
|---|---|---|---|
| 74 | [07](07-presentation.md) — **a system button's question, over a network** | small | Menu asks first and names its question through `{asker.…}`; the offer is shared state, so across a network it goes to the seat to play, not the one that clicked. Untested, and a loopback test says which |
