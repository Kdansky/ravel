...
| [07](07-presentation.md) | Presentation and its gestures | **gaps 1–8, 10 and fill shipped.** Gap 7 turned out to be a game file rather than a word: `games/system.json` is merged into every game, so the buttons are ordinary cards. Left: an offer of fifty-one, the log's `{name}`, and a Yes that names its question |
...
| [39](39-spellstorm-at-the-table.md) | Spellstorm at the table | **written up, not started.** The rules play; a player cannot follow them without hovering. The seat card's numbers, Initiative, the weather flip and damage — and an opponent's turn arriving over the network with no beats at all, filed under [02](02-between-two-states.md) |
...
|---|---|---|---|
| 74 | [07](07-presentation.md) — **a Yes that says what it is saying yes to** | small, and a word to agree first | Menu asks first now, all in `system.json`, but the dialog reads Yes / No with no question: a label cannot reach the card that asked. A reserved name beside `{owner}`. Also open: over a network the question goes to the seat to play, not the one that clicked |
| 75 | [40](40-reconnecting.md) — **picking a dropped game back up** | medium | a crash on either side of a network game ends it today, though the survivor still has everything. Mostly a test and a seating fix: seats follow host/guest, so a surviving guest who re-invites would be player one. Here because networked play just became seated, and the seating code is fresh |
...
| 72 | [39](39-spellstorm-at-the-table.md) — **weather and damage, noticed** | small, and a question first | damage already floats and bursts on the seat card, so ask what was missing before building: too small is `effect:` with a scope on the victim, not seen is 73. The weather flip wants a beat shown large |
| 73 | [02](02-between-two-states.md) — **the opponent's turn lands without a beat** | medium, and a design decision | every animation is recorded only for a click at this screen; a networked state is applied whole, so the waiting player never sees a flight, float or burst. Below 71 because it is engine work and hot-seat play has none of it |
| 24 | [16](16-the-player-at-this-screen.md) — **chat with the peer, and a name** | medium | asked for again, and on its own this time: a wire kind beside `H` that changes no state, and a text field — the input surface is still the decision. After 75 because a chat nobody can reconnect to is the smaller loss; the name can follow rather than ship with it |
...
**The validator stays one function.** Declined 2026-09-06. `validate.M.check` is
2,702 lines and 27 nested closures sharing `warn`, `claim` and four tag
