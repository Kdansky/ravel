...
| [41](41-a-name-for-a-list-of-actions.md) | A name for a list of actions | **shipped as a verb with a body** — `action` in place of `does`, arguments as `param1`… by position, stat changes inside it the verb's own so `adjusts` reaches them. A verb takes gates too, and Spellstorm's gated rules cards moved into verbs. What stays on a rules card is walked for its own state or owns an offer's answer. Left: Puzzle Strike's and Arnak's rules zones that are an if, and the 18 phase hooks, which are a moment rather than a call |
...
| [46](46-one-list-of-conditions.md) | One list of conditions | **shipped** — `needs` keyed by what failing it does: `req`, `where` (moved off targets, move rules and reveals), `fizzle`, `event`, and named gates that `name?` lines sit behind, asked once and kept. Replaced the inline if/do, and fixed Swamp Silt. A verb takes gates since. Left: a phase's `actions` and an end condition's `then`, when a game asks |
...
|---|---|---|---|
| 93 | [41](41-a-name-for-a-list-of-actions.md) — **Puzzle Strike's and Arnak's rules zones as gated verbs** | small, a migration, no word | seven offscreen zones of one-if cards — ante, combine, three upgrades, height, piggy; market, win, temple — each one entry once a verb takes gates, checked by seeded play as Spellstorm's were. Cheap and it lands while the word is fresh |
