...
| [20](20-puzzle-strike.md) | Puzzle Strike | medium | **built and playing** — character select over all ten base characters, the bank as a grid of counted stacks, ante/act/buy/cleanup, the mid-draw reshuffle, crashing, combining, Panic Time, and a loss condition asked only at your own turn's end. The one predicted gap (a card played out of turn) is still cut. The whole component catalogue is transcribed — 24 base chips, 24 Shadows chips, 60 character chips, 3 promos. Four findings the research missed: a scope cannot see a hand *again*, only a player phase can hand the turn over, the first `seat: "next"` selects seat one, and a condition about the **targets** is a `challenge`, not a `needs`. **Playing it then found what the validator could not** — the board was correct and unreadable — and six more engine pieces came out of that: badges on cards in a hand, four icon shapes and a badge that is a shape alone, `show:` + `chosen` for reading somebody else's hand, the word `optional` and a No choice button, `refill_from` on a zone (which deleted the draw loop and fixed Draw Three on a short bag), and two zone labels read off the engine. A card with nothing to run is no longer a move. **Then the owner caught two rules read wrong**: the piggy-bank icon is not "+1 buy" (there is no second buy in the game) but the keep-a-chip-through-cleanup option, and arrows have colours that must be spent before the plain one — which took one more engine piece: **`pays_for` on a stat** — "a plain arrow can be spent as a red one", said once where the resource lives rather than as a list of ways to pay copied onto every card. Which pool drains is then a matching the engine solves (most constrained demand first, own stat before any substitute), which is Magic's "4 generic and 3 red" and Splendor's gold as well |
...
| [28](28-a-zone-by-its-parts.md) | A zone by its parts | large, mechanical | **shipped whole** (2026-08-26). **Reopens [06](06-schema-and-types.md) gap 1 on the condition that gap named**: a game wanted a combination no `type` offers, and Puzzle Strike's ongoing row is it — an unbounded face-up row of cards that are *in play*. It took `hand`, and nine rules that read "in play" as `zone_type == "grid"` stopped seeing it. Five fields where there was one word: `layout` (`stack`/`row`/`grid`), `visibility`, `reach`, `use`, `status`. Refused as tags in 2026 and still refused as tags, for the reason 06 wrote down. `type` and the six shape tags are gone; 292 zone declarations, four generators and both documents moved with them, and three defaults come from a neighbouring field so the corpus got *shorter*. Found Bubble Shield relying on the ongoing row being a "hand" |
...
**What did turn up is a scope that cannot see a hand — now `@everywhere`.**
`count:<tag>` searches grid zones only, so a tag counted across hidden zones had
no single condition. That shipped as the `@everywhere` scope word (every card in
any zone, owner word and measuring forms composing), and measuring it corrected
the story it came in on: *naming a zone already reaches a hand, a pile and a
deck*, so only the **bare** tag was ever board-only. The two idioms the games
found were therefore never workarounds for this one word — "whose hand holds the
rocket 4" is irreducibly per-seat (which seat, not how many), so *walk the seats,
write a stat, read a computed tag* stays the right shape; and *narrow with the
zone and count the tag* was always available. `@everywhere` answers only the
question none of them could: **how many, anywhere, without naming the zones**.
...
| ~~1~~ | ~~[06](06-schema-and-types.md) — **a tag scope cannot see a hand, or a deck**~~ | — | — | **shipped as `@everywhere`** — a reserved scope word for every card in any zone, hands and decks included, composing with the owner word and every measuring form (`count:gem@mine.everywhere`). The default stays board-only on purpose (a rule must not read a hand it cannot see); this is the deliberate opt-in. **Measuring it shrank the gap**: naming a zone by key *already* reaches a hand, a pile and a deck (`count:gem@mine.discard` reads a pile today) — only the *bare* tag and tag scope were board-only. So the two workarounds were never for this: The Crew's commander is irreducibly per-seat (whose, not how many), and Puzzle Strike's gem pile is a grid for **display**, not for the scope. No game migration; `@everywhere` is the general answer for the one unsayable thing — a tag counted across hidden zones without enumerating them |
