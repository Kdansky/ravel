...
| [01](01-boardgames.md) | Any board game as JSON | **the ladder.** Lost Cities, chess and checkers shipped. The jumped square needed no word — a pattern is already a scope, and the anchor follows the piece — and the chain turned out to be a route back to the same seat that now ends itself: forced capture is `aims:` read once per card through a computed tag, and the *Done jumping* button is gone. Left: Klondike's run moves, and triggers |
...
| [07](07-presentation.md) | Presentation and its gestures | **gaps 1–8, 10 and fill shipped.** Gap 7 turned out to be a game file rather than a word: `games/system.json` is merged into every game, so the buttons are ordinary cards. Left: an offer of fifty-one, which end of a card the title lives at, the log's `{name}`, and whose screen a system button's question goes to |
...
| [18](18-legends-of-runeterra.md) | Legends of Runeterra | **milestone 2 plays** — eighteen spells, three speeds, a graveyard, a response window after blocks, Give and Grant, Rally. Left: Deny (which wants a stack, not a window), the rest of the keywords, champions |
...
| [37](37-codex.md) | Codex | **built and playing**, all 330 cards in the file: red, green and blue run, black, white, purple and the two neutral specs are scaffolding. Sacrifice is closed both ways, and so is the Truth spec: an Illusion dies of being aimed at, which is one line on a tag now. Left: three heroes a side, which is the rulebook's own game and the beginner's one is what ships; Macciatus and the two cards that hand the word out — an owner scope anchored on `@self`, and granting a tag at all — forecast, and three small words the box asks for 33 times between them |
...
| [41](41-a-name-for-a-list-of-actions.md) | A name for a list of actions | **shipped as a verb with a body** — `action` in place of `does`, arguments as `param1`… by position, stat changes inside it the verb's own so `adjusts` reaches them. Left: moving six games off their rules zones, and the gated and repeated routines a body cannot say |
| [42](42-an-if-inside-an-action-list.md) | An if inside an action list | **open, blocked on the grammar.** A condition may contain a colon, so `when:<cond>:<action>` has no reading. 44 Spellstorm abilities and 9 Codex ones exist only to carry a gate — and `activate_zone` reading a phase-run ability's `needs` as an if is the same gap, ~220 abilities across six games |
| [43](43-what-the-files-already-say-twice.md) | What the files already say twice | **shipped** — the `spell` tag carries the play, `tier_req` starts at 1, the redundant `subject` lines are gone, and a doubled `destroy` or `power_up` is one line with a count. Spellstorm lost 5 KB and says the same thing. Left: a badge that can only name a stat, recorded with one customer and no recommendation |
| [44](44-ghost-stories.md) | Ghost Stories | **built and playing**, four seats at Initiation level, all 65 cards. Village and boards are one 5x5 grid, so reach and movement are two patterns and no arithmetic; a Taoist rides its tile. Left: the two-ghost exorcism, tokens lent across a tile, the eight Taoist powers, the four seats sitting round the board rather than stacked beside it, and neutral boards for fewer than four players |
| [45](45-untrusted-input-past-the-file.md) | Untrusted input past the game file | **the file is done** — `shape.lua` holds it to its types and the fuzzer finds nothing. Left: a network state's entities are not checked, the live template editor writes anything, one test that fails alone, and two more blocks that could leave `flow` |
...
|---|---|---|---|
| 77 | [41](41-a-name-for-a-list-of-actions.md) — **what verb bodies cannot say yet** | medium, and a design first | four games moved onto verbs with no change in play. What is left in rules zones is what a body cannot say: a routine behind a `needs`, and one run once per point of something. `GIVE(kind)` waits on a parameter being a whole word |
...
| 92 | [42](42-an-if-inside-an-action-list.md) — **`activate_zone` stops reading `needs`** | medium, a migration over six games | a phase-run ability that fails its `needs` silently skips, where a `needs` everywhere else decides whether a move is offered — one word, two meanings. About 220 step abilities lean on it (Spellstorm, Ghost Stories, Codex, Grimm, Arnak, LoR's `spill`), and each needs 78's inline gate to move into, so it sorts straight after 78 |
| 87 | [44](44-ghost-stories.md) — **the four seats sitting round the board** | small, generator only | today the four seat bands stack in one left-hand column and the right column stops short of the bottom. Wanted: a seat to a side, its four zones adjacent, making a ring round the village; the action tray narrower and on the left; the right column full height. No engine change and no word — `pos` is already per-seat, and the hand branch of `render.laid_out` picks cells closest to square, so a `row` zone comes out vertical in a tall rect by itself |
| 86 | [07](07-presentation.md) — **which end of a card the title lives at** | medium, and a word to agree first | Codex is where the bottom text band runs out: the most badges of any game and the longest text, with the title giving way to the badge run inside the same band. Title to the top under the cost badge, badges all to the bottom, is two bands that never negotiate. Three decisions in one and twenty-one games drawing through the path, so it wants a look at a real Codex hand before a word is chosen |
| 85 | [37](37-codex.md) — **three heroes a side** | medium, and a rules read first | the rulebook's own game is three specs of one colour; the file deals one, which is the beginner's game. The pick becomes six colours rather than eighteen specs, the shared starter ten leaves the spec card, `hero_wait` moves off the player onto the hero, and `ripe` stops being `max:level` over the whole side — which is the one engine question, whether an `each` can read the card it is standing on. The codex zone itself needs nothing |
| 80 | [44](44-ghost-stories.md) — **the two-ghost exorcism** | medium, and a word to agree first | a corner tile faces two spaces and one roll may clear both, splitting the dice as the player likes. What an exorcism is worth is a `compute` read fresh each time, so a die spent on the first ghost is still there for the second — the fix is for a face to be *consumed*, which is a third zone and an ordering question. It is also the fix for a resistance printed in several colours spending one white twice, so two rules for one piece of work |
| 81 | [44](44-ghost-stories.md) — **the eight Taoist powers** | medium each, no word to agree | two are a line apiece, two are a second pass through the Yang phase, and one is a fourth die. They are what the four ghosts that switch a board's power off are *for*, so the ghosts are already carrying a tag nothing reads |
...
| 88 | [45](45-untrusted-input-past-the-file.md) — **a state from the network, checked before it is believed** | medium | a peer can send a card whose `hp` is a string or a zone whose `cards` is a number, and the receiving game crashes on the first read. `net.restore` checks the phase stack and nothing inside an entity. An entity shape in `shape.lua`'s terms, ids checked against the array they came in, and the message refused whole before `entity.restore` — the door already exists. Here because it is the one crash a game file can no longer cause and a message still can |
...
| 90 | [45](45-untrusted-input-past-the-file.md) — **`test_codex_shape` alone** | tiny | reads `declaration.G` before `flow.init` replaces it, so it passes only after another test has loaded a game. Move one line; then run every integration test on its own once, to find the others |
| 89 | [45](45-untrusted-input-past-the-file.md) — **the live editor, held to the shape** | small | `cards.edit` writes any value onto a template, the one path into `G` that skips `shape.lua`. Developer-only (`RAVEL_DEBUG=1`), so below the network. Check the one field against `shape.SPECS.cards` and refuse |
| 91 | [45](45-untrusted-input-past-the-file.md) — **the ending and the offers out of flow** | small, and optional | `flow.lua` is 1,600 lines after `costs` and `stack`. The ending (~90 lines) reads state and nothing else; the offers (~130) share `clear_offer` with `play_card`. Not worth doing for its own sake — do it when next in there |
| — | [18](18-legends-of-runeterra.md) + [01](01-boardgames.md) gap 5 — **triggers and champions** | large | not ranked as one item on purpose. Spells and speeds shipped; what is left is Deny, spell mana, the mulligan, and a hand bounded at ten |
