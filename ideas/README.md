...
| [32](32-a-third-player.md) | A third player | **the phase half shipped.** `type: "turn"` is a phase whose body is other phases, run once per player with `seat: "each"` and in the order `order` names — so a step every player takes is one declaration. Sixteen of Spellstorm's twenty-five phases were per-seat copies; a third seat costs no phases at all now. Left: a word narrower than `enemy`, which means "not me", and a group that starts with whoever is already up |
...
| 18 | [32](32-a-third-player.md) — **a word narrower than `enemy`** | small, and a word to agree first | `enemy` is right as a *filter* — "every creature an opponent controls" wants all of them — and a trap as a *subject*: Spellstorm writes `@enemy.player` 53 times meaning *the* opponent, and with three seats every one becomes a table-wide effect. The proposed spelling is **`opponent`**, meaning the sole other seat and refused outright at three, which turns 53 silent doublings into 53 authoring-time errors — but it still says nothing about *which* opponent, so a three-seat game needs one of the relational words beside it |
