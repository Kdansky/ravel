...
| [20](20-puzzle-strike.md) | Puzzle Strike | **built and playing**, the whole box drafted. Buying has a floor and no ceiling since, and three of the four chips that trade one in for another offer the bank instead of borrowing a buy phase. Left: a short list of engine features, Signature Move's second half, six gems that still break into the void rather than the bank, and the per-chip state is in the game file |
...
| 5 | [20](20-puzzle-strike.md) — **Signature Move's second half, and six gems that break into the void** | small each | **The buy ceiling is gone**, and three chips trade one in through an offer of the bank rather than a borrowed buy phase — one chip judged against one number, landing in the hand Training Day names. What is left of the row is two content faults. **Signature Move** never plays the chip it fetched, and its `DEV:` line blames a missing capability that exists: `copy:target:play` is a play without a cost, and X-Copy already uses it. The second question needs a second asker — a rules card the chip activates — and one decision, whether the search reaching hand and table as well as bag and discard is close enough. **The void gems**: breaking a gem is still `move_target_to:void`, so the broken 2s and 3s sit offscreen for ever instead of going back in a box that now takes them |
