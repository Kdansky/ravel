...
| [20](20-puzzle-strike.md) | Puzzle Strike | **built and playing**, the whole box drafted. Buying has a floor and no ceiling since. Left: a short list of engine features, Signature Move's second half, six gems that still break into the void rather than the bank, and the per-chip state is in the game file |
...
| 5 | [20](20-puzzle-strike.md) — **Signature Move's second half, and six gems that break into the void** | small each | **The buy ceiling is gone** — the rulebook states a floor and nothing above it, so `buys` went with it and *End turn* reads the tally. What is left of the row is two content faults. **Signature Move** never plays the chip it fetched, and its `DEV:` line blames a missing capability that exists: `copy:target:play` is a play without a cost, and X-Copy already uses it. The second question needs a second asker — a rules card the chip activates — and one decision, whether the search reaching hand and table as well as bag and discard is close enough. **The void gems**: breaking a gem is still `move_target_to:void`, so the broken 2s and 3s sit offscreen for ever instead of going back in a box that now takes them |
| 6 | [31](31-either-of-two.md) — **`or` between conditions** | small, and now with customers | **moved up: the honest customers turned up.** Removing Puzzle Strike's buy ceiling left two faults that are one condition each and cannot be written as one comparison — a borrowed shopping phase that should ration to the chip it granted ("my own buy phase, or I still have an allowance"), and the rulebook's own escape from the buy-one rule when the Wound stack is empty, which currently deadlocks the turn. `meets_all` is the one function fourteen consumers share, so whatever spelling wins lands once |
| 7 | [07](07-presentation.md) gap 7 — **a word for where the buttons go** | small | eight games have each invented their own chrome in raw fractions, and an author must answer "where do the buttons go" before putting a button anywhere. The cheap version is a named region resolved in `zones.resize`, one branch, keeping the model that a button is a card in a zone. Unblocked: item 2 shipped and left the buttons at 28px, which is this question's answer to give |
| 8 | [07](07-presentation.md) — **where the numbers are read, and whether zeros are** | medium, and a word to agree first | `draw_stats` is hard-anchored to the top-right corner in pixels, so any game using that corner has its readout printed over it. Two independent halves: a place, which needs a new field and therefore consent; and zeros, where `badge_zeros` already exists for the card end of the same question |
