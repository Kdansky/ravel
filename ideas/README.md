...
| [02](02-between-two-states.md) | An animation between two states | **stage 1 shipped**: a click's visible steps are recorded in order and played back a beat at a time, with input held and a second click speeding the run up rather than dropping it. Left: the presented state, so numbers and flips agree with the cards |
| [03](03-a-move-out-of-a-stock.md) | A move out of a stock | **not started, one word to agree.** A supply keeps one card per kind, so nothing can be moved out of one — a crash is written as a `fill` plus a separate decrement, which is why it could not be animated and cannot be checked. `take` is `move` for a source that counts instead of keeps |
...
| [20](20-puzzle-strike.md) | Puzzle Strike | **built and playing**, the whole box drafted. Left: a short list of engine features, the buy count and Signature Move's second half, and the per-chip state is in the game file |
...
|---|---|---|---|
| 1 | [02](02-between-two-states.md) — **stage 2, the presented state** | medium | **stage 1 shipped**: `game/stage.lua` keeps a click's order and plays it back, and a draw off a pile travels instead of appearing. What is left is that the renderer still reads the live registry, so a number changes while its card is in the air and a destroyed card cannot be animated out at all. The fix is one accessor over 21 sites in `render.lua`, and each presented state is a real snapshot — which is why stats, flips and pile counts come along for free |
...
| 3 | [03](03-a-move-out-of-a-stock.md) — **`take`** | small, and a word to agree first | ranked beside item 4 because they are the same pass over supplies: 28 closes the box, this opens it. **The count on `move` shipped** and was the half needing no new word. `take` is the half that needs consent, and it pays for itself twice: 96 sites in Puzzle Strike stop being two statements that can drift apart, and [02](02-between-two-states.md) stops guessing where a conjured card came from |
...
| 5 | [20](20-puzzle-strike.md) — **two chips that do not do what they print** | small each | **Buying more than one chip a turn** is a bug: the buy ability costs `buys: 1` and the phase sets `buys` to 1, so money on the table cannot be spent. **Settled** — "+2 buys" on Always in Control was a transcription error for "+2 piggy", so nothing argues for a ceiling: buy as many as you can afford, at least one, and *End turn* ends the phase. Generator edit, plus that chip's own text. The piggybank rename rides along. **Signature Move** never plays the chip it fetched, and its `DEV:` line blames a missing capability that exists: `copy:target:play` is a play without a cost, and X-Copy already uses it. The second question needs a second asker — a rules card the chip activates — and one decision, whether the search reaching hand and table as well as bag and discard is close enough |
| 6 | [07](07-presentation.md) gap 7 — **a word for where the buttons go** | small | eight games have each invented their own chrome in raw fractions, and an author must answer "where do the buttons go" before putting a button anywhere. The cheap version is a named region resolved in `zones.resize`, one branch, keeping the model that a button is a card in a zone. After item 2, because a filled button changes what a sidebar has to be shaped like |
...
| 8 | [09](09-composition.md) — **`include`, then a base file of patterns** | medium | worth re-opening with the pause's two findings settled. Four generated games now, whose files are only maintainable *because* of the generator — the argument arriving from a new direction |
| 9 | [25](25-derived-stats.md) — **re-read, not built** | small + one decision | what is left is whether a stat that keeps itself is worth anything *once a formula can sit at its use site with a name on it*. Answer it with The Crew's `weigh` migrated. The one thing a compute cannot do is clamp, and Splendor's pricing rests on `min: 0` |
| 10 | [01](01-boardgames.md) gap 1 — **the square a move passes over** | medium | a jump takes the piece it flies past, and nothing can name that square. Castling-through-check asks for the same word; en passant no longer does, having shipped as `where` |
...
| 15 | [07](07-presentation.md) — **an offer of fifty-one** | small, after 2 | the draft may only look bad because its two buttons are 23px squares. Look again with item 2 in, then decide whether `layout: "page"` — which already draws the reveal overlay — should serve an offer too |
