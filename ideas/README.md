...
| [36](36-the-validator-as-one-function.md) | The validator as one function | **not started.** `M.check` is 2,702 lines and 27 nested closures sharing one `warn`, one `claim` and four tag universes by capture — so no check can be read or exercised alone, and every new format word is edited into the middle of it |
...
| 29 | [35](35-a-board-that-fits-the-game.md) — **`set_name`'s customer** | tiny + one reading | the verb shipped as item 8 and nothing writes it: the wizard whose pick was the entire reason for it still leaves the seat reading `"Player One"`. One line on `pick_action` in `make_spellstorm.py`, and one question — what `"text": "{name}"` shows before a wizard is picked. An engine word no game spends is a word the format charges for twice |
| 30 | [36](36-the-validator-as-one-function.md) — **the validator's shape** | medium, and a size to measure first | it cleans up the file the review pass just spent itself in, and it is under items 9, 11 and 14 — each of those adds a format word, and a word is a check edited into the middle of 2,700 lines. Nothing a player sees, which is why it has never been ranked; the argument for it now is that three ranked items are about to pay its cost |
