...
| 3 | [03](03-a-move-out-of-a-stock.md) — **`take`, and a count on `move`** | small each, and a word to agree first | ranked beside item 4 because they are the same pass over supplies: 28 closes the box, this opens it. The count on `move` is the cheap half and needs no new word — `destroy` takes one and `move` does not, which is why a card that sends two gems across the table is written as a death and a birth. `take` is the half that needs consent, and it pays for itself twice: 96 sites in Puzzle Strike stop being two statements that can drift apart, and [02](02-between-two-states.md) stops guessing where a conjured card came from |
