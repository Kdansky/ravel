...
| [41](41-a-name-for-a-list-of-actions.md) | A name for a list of actions | **shipped as a verb with a body** — `action` in place of `does`, arguments as `param1`… by position, stat changes inside it the verb's own so `adjusts` reaches them. Left: the gated routines a body cannot say, which wait on [42](42-an-if-inside-an-action-list.md)'s if |
...
| 77 | [41](41-a-name-for-a-list-of-actions.md) — **the gated routines left in rules zones** | medium, a migration | once per point and a verb over a kind both turned out to need no word (`copy`'s count reads a stat; a kind is a tag, so `junk_pile.param1` finds the pile). What is left is every routine behind a `needs`, which moves into 78's if — so it sorts after 78 and beside 92, which is the same migration seen from `activate_zone`'s side |
