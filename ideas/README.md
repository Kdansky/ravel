...
| [35](35-a-board-that-fits-the-game.md) | A board that fits the game | **the seat's own name shipped**, as `set_name` over a `{name}` template plus a `name` field to start it. Left: the shelf — several mutually exclusive zones on one rect, spelled as `pos` naming a zone — which has two customers, one of them a comment in `make_spellstorm.py` asking for it by name. `shown_when` waits behind it |
...
| 11 | [35](35-a-board-that-fits-the-game.md) — **the shelf: several zones, one rect** | medium, and a word to agree first | `validate.lua:2482` refuses overlapping zones, and `make_spellstorm.py:1378` carries a comment saying in as many words that `commit` and `battle` *would share a rect if they could* — the two are never both open, so the reveal would be the card turning over where it lay. The spelling to argue about is `pos` naming a zone instead of four numbers, which costs no key and gives the overlap check the sentence it is missing. It is also what a character's furniture needs once one wizard wants more than the sidecar's single row, and what `shown_when` should wait behind |
