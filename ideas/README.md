...
| [07](07-presentation.md) | Presentation and its gestures | **gaps 1–6, 8, 10 and fill shipped.** Gap 10 deleted the HUD rather than relocating it: a seat must be visible now, and hovering it already lists its stats. Left: a word for where the buttons go, and an offer of fifty-one |
...
| [35](35-a-board-that-fits-the-game.md) | A board that fits the game | **not started.** Spellstorm's clutter, minus the two halves already shipped. The shelf — several mutually exclusive zones on one rect, spelled as `pos` naming a zone — has two customers and one of them is a comment in `make_spellstorm.py` asking for it by name. `shown_when` waits behind it, and the seat's own name is a third question on a different axis |
...
| 19 | [09](09-composition.md) — **a seat count as a module, over Spellstorm** | medium | the test `include` is waiting for, and the only thing that would give the word a user. Adding *and* removing a seat, because the two fail differently. Spellstorm is two seats, 6 per-seat zones and five `_1`/`_2` phase pairs, so it exercises every merge rule at once — and its file is generated while the module would be hand-written, which is the split `include` exists to make possible. The phases it was blocked on are gone — Spellstorm has no `_1`/`_2` pair left — so what is left is the merge itself |
