...
| 1 | [22](22-the-crew.md) — **the seat a scope points at** | medium | small | **half shipped**: `@owner_of.<scope>` reads it — the seats owning what a scope names, answered as their own cards, with the acting card's own seat as the bare form. A prefix rather than a word of its own, since the words after `@` are separated by `.` and a colon would arrive as a second action argument. What is left is the writing half — `set_active_seat:<scope>`, a `G.seat_index` lookup over the answer the scope already gives — and it is what The Crew cannot be built without: the trick winner leads the next trick, and LoR's attack token holder is supposed to act first |
