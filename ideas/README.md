...
| [34](34-an-opponent.md) | An opponent | **step 1 shipped.** The board-game kind: a seat the engine plays. `legal_moves` is `game/opponent.lua`'s `M.legal()` now, moved verbatim out of the test harness with the suite unchanged as the proof — a random opponent is that list and one `math.random`. Left: driving it from a seat, which is where `net.lua`'s wrappers get their say; and after that a way for a game file to say what winning looks like as a *number*, which is what everything above random waits on |
...
| 33 | [arnak](arnak/design.md) — **who spent a card's exhaust** | small, and **no word after all** | the engine records that a card is exhausted and not by whom, so Arnak's Fear rule — *your* archaeologist coming home from a guarded site — could not be written. It needs no engine change: the ability paying the exhaust stamps the spender itself, `stat_set:spender@self:sum:side@mine.player`, and the rule asks it back with both sides live. Checked on a two-seat fixture — offered to the spender and to nobody else. What is left is writing it into Arnak, and the cleanup wipe it needs because a readied card keeps a stale stamp |
| 15 | [01](01-boardgames.md) gap 1 — **the square a move passes over** | medium, and **half of it is already there** | a jump takes the piece it flies past, and nothing can name that square. Not a reaction: `reactions.lua` decides *who may answer an event*, and a capture is nobody answering. But a move rule's `where` anchors its patterns on the square being considered (`predicate.pattern_slots` reads `ctx.anchor`), so *asking* about the mid square is writable today as one move rule per direction, each with a fixed offset. What is missing is only the **taking** — an action naming that square. Castling-through-check wants the asking half and may already be done |
