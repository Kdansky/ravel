...
| 31 | [09](09-composition.md) — **provenance in validator messages** | small | every game is a merge now, so `card 'sys_log': ...` is a complaint about a file the author never opened, printed against all seventeen of them. The map that would answer it already exists: `fold` keeps `out.from`, key → source file, to name both sides of a collision, and `M.read` throws it away four lines later. Carrying it into `parse` and appending `(from system.json)` is the use it was built for, and it is the cheapest thing on this list |
