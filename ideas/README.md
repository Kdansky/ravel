...
| [33](33-a-row-that-closes-up.md) | A row that closes up | **not started.** Arnak's card row: position on a shelf is how old a card is, and nothing says so. Two words — `compact` and a cell on `draw_from` — and the staff becomes a card standing in a seven-wide grid |
...
| 12 | [33](33-a-row-that-closes-up.md) — **`compact`, and a cell on `draw_from`** | medium | Arnak exiles an arbitrary card from its market every round, because `destroy` sorts by entity id and an id says nothing about how long a card has been on display. The fix is not a field: a shelf has no order the format can address. Two thirds of the rule turns out to be free once the moon staff is a card standing in the row — `destroy:beside` is the rulebook sentence — so what is left is sliding the row shut and dealing into the end it opens. Worth more than one game: every market with a river has the same shape |
