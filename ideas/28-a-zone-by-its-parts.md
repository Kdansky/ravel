...
| `status` | `board` · `exile` · `offer` | `board` on a grid, else `exile` | what standing a card here has in the rules |
...
`status`, and it is why the field is not `in_play` — a question that stops being
