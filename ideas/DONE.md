...
it is by far the fastest way to check anything visual or browser-specific, **and
it is the only way to see the interpreter the game actually ships on** — love.js
is Lua 5.1 where everything local is LuaJIT or 5.4. A `goto` once passed the
whole suite and died on the first browser frame; this is what proved it, by
putting the bug back and watching the error screen return.
...
| **`goto` shipped and the browser refused it.** The engine's dialect is Lua **5.1**, because love.js is LÖVE compiled against it — but every interpreter the suite runs under implements `goto`: LuaJIT has it as a 5.2 extension, Lua 5.4 has all of it, and desktop LÖVE is LuaJIT. So `goto continue` in `flow.can_afford` passed 1210 assertions on two interpreters, booted fine on the desktop, and died on the first frame in a browser with a syntax error naming a line nobody had touched. | `tests/run.lua` greps all 28 modules for 5.2-isms — `goto`, `::labels::`, `//`, `<<`, `>>` — after stripping comments and long strings, since netlink's embedded JavaScript is full of `//`. |
| **An opponent's hand could be read three ways.** `zones.visible` hid the faces on the board, and nothing else asked: right-clicking a card opened its detail view, right-clicking the hand browsed every card in it, and ctrl+hover printed the card's key — which is the whole of what a hidden hand hides. Drawing backs is half of hiding something; every path that *reads* a card is the other half. | `zones.peekable(z)` is the container's twin of `visible(c)`, and all three paths ask one of the two. The browser draws backs for anything it may not show, so no future caller can undo it either. |
