...
| Artifact cards: the powered effect destroys the card on use | **exists** — `purge:self` at the end of an ability's `action` list |
| Wound cards: unplayable, count only against hand size, enter hand as combat damage | **expressible** — a `token`-tagged card with no `play` block, created via `create:<zone>:<card>:<n>` inside the damage-assignment action list |
| Source: a shared pool of colored mana dice, day/night restricts which colors are legal to take, returned and rerolled at round end | **expressible** — a shared (non-`per_seat`) zone of color-tagged token cards; day/night legality is the same `phases` tag-grant as for cards; "reroll" is `move` + `shuffle` + redeal, the documented "roll / draw randomly" idiom |
