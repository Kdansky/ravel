...
| Armor as a flat damage-to-defeat threshold, Fame awarded on defeat | **exists** — a stat comparison (`needs`/routing `at_least`) plus `gain_stat:fame:n`, identical in shape to any boss-HP check |
| Fortified sites/enemies: only Siege attacks work before blocking, and a Reputation cost to even attempt | **exists** — `receive.needs`/`play.needs` tag-gating on a `fortified` tag, plus an ordinary `lose_stat` in the action list |
