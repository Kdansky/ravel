...
Every numeric slot accepts a number, a measure (`count:<tag>`, `sum:<subject>`, `max:`, `min:`, `card:`) — the number of cards on grid zones carrying that tag: `stat_gain:gold:count:economic` — or the key of a **compute** the ability named. One amount rule, everywhere. A bare word that is none of those is a typo and the validator says so; it used to read as zero and run silently.
