...
Splendor: It seems weird that every card has so many 0 value stats. Why is that? Can we get rid of them by having some sort of global rule that allows cards to init any stat (or a list of stats) to 0 when required?
